...
export default function AgendarCita() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!correoValido || !telefonoValido || !placaValida) {
      Swal.fire({ title: "Datos inválidos", text: "Por favor, ingresa datos válidos.", icon: "error" });
      return;
    }

    if (!captchaToken) {
      Swal.fire({ title: "Verificación requerida", text: "Completa el hCaptcha antes de enviar.", icon: "warning" });
      return;
    }
    ...
    try {
      const response = await fetch(`${API_URL}/citas`, {
        ...
        body: JSON.stringify({ nombre, correo, telefono, fechaCita, horaCita, placa, cdaSeleccionado, captchaToken }),
      });
      ...
      Swal.fire({
        ...
        html: `<b>Nombre:</b> ${nombre} <br><b>Fecha:</b> ${fechaCita} <br><b>Hora:</b> ${horaCita} <br><b>Placa:</b> ${placa} <br><b>CDA:</b> ${cdaSeleccionado}`,
        icon: "success",
      ...
      <div className="agendar-container">
        <div className="card">
          ...
          <p className="info-text">
            Ingresa correctamente todos los datos. La fecha y la hora serán validadas en el sistema. 
            Asegúrate de que la placa sea correcta y esté escrita sin espacios.
          </p>
